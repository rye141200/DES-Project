...
int main(){
    ...
    // printHex(expansionPermutation(0x2EBA0635).value);
    //printHex(XOR48Bits(Unsigned48Bits(0x95d5f400c1aa),Unsigned48Bits(0xB02679B49A5)).get());
    // printHex(permutation(0x2487B114));
    //printHex(XOR32Bits(0xA34033B0,0x1CCD0EA8));
    uint64_t S1 = (0x1200FF00FF00 & 0xFC0000000000)>>42;
    
    uint64_t S2 = (0x1200FF00FF00 & 0x03F000000000) >>36;
    printHex(S2);
}
...
Unsigned48Bits expansionPermutation(uint32_t RiPrev){   
    ...
    for(int i =0;i<48;i++){
        ...
        uint64_t bit = EXTRACTBIT(RiPrev,31-bitPosition); //! 31 because C++ treats hexadecimals as little endian
        expanded.set(expanded.get() | (bit<<(47-i)));
    }
    return expanded;
}
Unsigned48Bits XOR48Bits(Unsigned48Bits expandedPermuted,Unsigned48Bits Ki){
    return expandedPermuted.get() ^ Ki.get();    
}

uint32_t SBox(Unsigned48Bits XORedExpandedPermutedWithKey){
    //TODO
    uint64_t inputValue = XORedExpandedPermutedWithKey.get();
    //!1) Partition the input into 8 partitions
    uint64_t S1 = (inputValue & 0xFC0000000000) >>42;
    uint64_t S2 = (inputValue & 0x03F000000000) >>36;
    
    //!2) Handle the little endian representation by subtracting the bit position from 47, when extracting the bit
}

uint32_t permutation(uint32_t SBoxed){
    uint32_t permuted;
    for(int i =0;i<32;i++){
        int bitPosition = postSBoxPermutation[i]-1; //! 0 based indexing
